...
PERF_THRESHOLDS = {
    ...
    "min_metric_recording_throughput": int(
        os.getenv("PERF_MIN_METRIC_THROUGHPUT", "1000")
    ),
    "max_metric_retrieval_time_ms": int(
        os.getenv("PERF_MAX_METRIC_RETRIEVAL_MS", "1000")
    ),
    "max_workflow_time_ms": int(os.getenv("PERF_MAX_WORKFLOW_TIME_MS", "500")),
    "min_concurrent_connections": int(
        os.getenv("PERF_MIN_CONCURRENT_CONNECTIONS", "10")
    ),
    "min_max_throughput_ops_per_sec": int(os.getenv("PERF_MIN_MAX_THROUGHPUT", "50")),
    "max_extreme_load_memory_mb": int(
        os.getenv("PERF_MAX_EXTREME_LOAD_MEMORY_MB", "1000")
    ),
    "min_extreme_load_success_rate": float(
        os.getenv("PERF_MIN_EXTREME_LOAD_SUCCESS_RATE", "0.5")
    ),
    "min_sustained_throughput_ops_per_sec": int(
        os.getenv("PERF_MIN_SUSTAINED_THROUGHPUT", "5")
    ),
    "min_sustained_success_rate": float(
        os.getenv("PERF_MIN_SUSTAINED_SUCCESS_RATE", "0.7")
    ),
...
from src.api.services.mcp.tool_binding import (
    ToolBindingService,
    BindingStrategy,
    ExecutionMode,
)
...
from src.api.services.mcp.service_discovery import (
    ServiceRegistry,
    ServiceType,
    ServiceInfo,
    ServiceStatus,
...
class TestMCPLoadPerformance:
    ...
    async def discovery_service(self):
        ...
        config = ToolDiscoveryConfig(discovery_interval=1)
    ...
    async def routing_service(self, discovery_service, binding_service):
        ...
        try:
            ...
            if "_setup_routing_strategies" in str(
                e
    ...
    async def test_single_tool_execution_latency(self, mcp_server, mcp_client):
        ...
        connected = await mcp_client.connect_server(
            "test_server", MCPConnectionType.HTTP, "http://localhost:8000"
        )
        if not connected:
            pytest.skip(
                "MCP server not available - requires running server on localhost:8000"
            )
        ...
        for i in range(100):
            ...
            try:
                result = await mcp_client.call_tool(
                    "get_inventory", {"item_id": f"ITEM{i:03d}"}
                )
                end_time = time.time()
                if result:
        ...
        ), f"Average latency should be < {PERF_THRESHOLDS['avg_latency_ms']}ms: {avg_latency*1000:.2f}ms"
        assert (
            p95_latency < p95_latency_threshold
        ), f"95th percentile latency should be < {PERF_THRESHOLDS['p95_latency_ms']}ms: {p95_latency*1000:.2f}ms"
        assert (
            p99_latency < p99_latency_threshold
        ), f"99th percentile latency should be < {PERF_THRESHOLDS['p99_latency_ms']}ms: {p99_latency*1000:.2f}ms"

        logger.info(
            f"Latency Stats - Avg: {avg_latency*1000:.2f}ms, P95: {p95_latency*1000:.2f}ms, P99: {p99_latency*1000:.2f}ms"
        )
        print(
            f"Latency Stats - Avg: {avg_latency*1000:.2f}ms, P95: {p95_latency*1000:.2f}ms, P99: {p99_latency*1000:.2f}ms"
        )
    ...
    async def test_concurrent_tool_execution(self, mcp_server, mcp_client):
        ...
        if "get_inventory" not in mcp_client.tools:
            pytest.skip(
                "Tool 'get_inventory' not available - server may not be properly configured"
            )
        ...
        for concurrency in concurrency_levels:
            ...
            success_rate = (
                len(successful_results) / len(results_list) if results_list else 0
            )
            ...
            print(
                f"Concurrency {concurrency}: {execution_time:.3f}s, {success_rate:.2%} success, {throughput:.2f} ops/sec"
            )

        # Assertions - only if we have results
        if 1 in results:
            assert (
                results[1]["success_rate"] > 0.9
            ), "Single execution should have high success rate"
        if 10 in results:
            assert (
                results[10]["success_rate"] > 0.8
            ), "10 concurrent executions should maintain good success rate"
        if 50 in results:
            assert (
                results[50]["success_rate"] > 0.7
            ), "50 concurrent executions should maintain reasonable success rate"
    ...
    async def test_throughput_under_load(self, mcp_server, mcp_client):
        ...
        while time.time() - start_time < test_duration:
            ...
            for i in range(10):  # Batch size
                task = mcp_client.call_tool(
                    "get_inventory", {"item_id": f"ITEM{completed_operations + i:06d}"}
                )
                tasks.append(task)
        ...
        print(
            f"Sustained Load - Duration: {total_time:.1f}s, Operations: {total_operations}, Throughput: {throughput:.2f} ops/sec, Success: {success_rate:.2%}"
        )
        ...
        ), f"Should maintain reasonable throughput: {throughput:.2f} ops/sec (threshold: {min_throughput})"
        assert (
            success_rate > min_success_rate
        ), f"Should maintain good success rate: {success_rate:.2%} (threshold: {min_success_rate:.2%})"
    ...
    async def test_memory_usage_under_load(self, mcp_server, mcp_client):
        ...
        for batch in range(100):  # 100 batches
            ...
            for i in range(50):  # 50 operations per batch
                task = mcp_client.call_tool(
                    "get_inventory", {"item_id": f"ITEM{batch * 50 + i:06d}"}
                )
                tasks.append(task)
        ...
        print(
            f"Memory Usage - Initial: {initial_memory:.1f}MB, Max: {max_memory:.1f}MB, Final: {final_memory:.1f}MB, Increase: {memory_increase:.1f}MB"
        )
        ...
        ), f"Memory increase should be reasonable: {memory_increase:.1f}MB (threshold: {max_memory_increase}MB)"
        assert (
            max_memory < max_memory_usage
        ), f"Maximum memory usage should be reasonable: {max_memory:.1f}MB (threshold: {max_memory_usage}MB)"
    ...
    async def test_tool_discovery_performance(self, discovery_service):
        ...
        for i, adapter in enumerate(mock_adapters):
            await discovery_service.register_discovery_source(
                f"adapter_{i}", adapter, "mcp_adapter"
            )
        ...
        print(
            f"Tool Discovery - Avg search time: {avg_search_time:.3f}s, Max search time: {max_search_time:.3f}s"
        )

        # Assertions
        assert (
            avg_search_time < 0.1
        ), f"Average search time should be fast: {avg_search_time:.3f}s"
        assert (
            max_search_time < 0.5
        ), f"Maximum search time should be reasonable: {max_search_time:.3f}s"
    ...
    async def test_tool_binding_performance(self, discovery_service, binding_service):
        ...
        await discovery_service.register_discovery_source(
            "mock_adapter", mock_adapter, "mcp_adapter"
        )
        await asyncio.sleep(2)
        ...
        print(
            f"Tool Binding - Avg binding time: {avg_binding_time:.3f}s, Max binding time: {max_binding_time:.3f}s"
        )

        # Assertions
        assert (
            avg_binding_time < 0.1
        ), f"Average binding time should be fast: {avg_binding_time:.3f}s"
        assert (
            max_binding_time < 0.5
        ), f"Maximum binding time should be reasonable: {max_binding_time:.3f}s"

    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_tool_routing_performance(
        self, discovery_service, binding_service, routing_service
    ):
        ...
        for i in range(100):
            ...
            decision = await routing_service.route_tools(
                context, strategy=RoutingStrategy.BALANCED, max_tools=10
            )
            end_time = time.time()
        ...
        print(
            f"Tool Routing - Avg routing time: {avg_routing_time:.3f}s, Max routing time: {max_routing_time:.3f}s"
        )

        # Assertions
        assert (
            avg_routing_time < 0.1
        ), f"Average routing time should be fast: {avg_routing_time:.3f}s"
        assert (
            max_routing_time < 0.5
        ), f"Maximum routing time should be reasonable: {max_routing_time:.3f}s"

    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_tool_validation_performance(
        self, discovery_service, validation_service
    ):
        ...
        print(
            f"Tool Validation - Avg validation time: {avg_validation_time:.3f}s, Max validation time: {max_validation_time:.3f}s"
        )

        # Assertions
        assert (
            avg_validation_time < 0.01
        ), f"Average validation time should be very fast: {avg_validation_time:.3f}s"
        assert (
            max_validation_time < 0.1
        ), f"Maximum validation time should be reasonable: {max_validation_time:.3f}s"
    ...
    async def test_service_discovery_performance(self, service_registry):
        ...
        print(
            f"Service Discovery - Avg discovery time: {avg_discovery_time:.3f}s, Max discovery time: {max_discovery_time:.3f}s"
        )

        # Assertions
        assert (
            avg_discovery_time < 0.1
        ), f"Average discovery time should be fast: {avg_discovery_time:.3f}s"
        assert (
            max_discovery_time < 0.5
        ), f"Maximum discovery time should be reasonable: {max_discovery_time:.3f}s"
    ...
    async def test_monitoring_performance(self, monitoring_service):
        ...
        print(
            f"Metric Recording - Time: {recording_time:.3f}s, Throughput: {recording_throughput:.2f} metrics/sec"
        )

        # Test metric retrieval
        start_time = time.time()
        metrics = await monitoring_service.metrics_collector.get_metrics_by_name(
            "test_metric"
        )
        retrieval_time = time.time() - start_time

        print(
            f"Metric Retrieval - Time: {retrieval_time:.3f}s, Metrics retrieved: {len(metrics)}"
        )

        # Assertions
        assert (
            recording_throughput > 1000
        ), f"Should record metrics quickly: {recording_throughput:.2f} metrics/sec"
        assert (
            retrieval_time < 1.0
        ), f"Should retrieve metrics quickly: {retrieval_time:.3f}s"
    ...
    async def test_end_to_end_performance(
        ...
        for i in range(100):
            ...
            decision = await routing_service.route_tools(
                context, RoutingStrategy.BALANCED, max_tools=5
            )
        ...
        print(
            f"End-to-End Workflow - Avg time: {avg_workflow_time:.3f}s, Max time: {max_workflow_time:.3f}s"
        )

        # Assertions
        assert (
            avg_workflow_time < 0.5
        ), f"Average workflow time should be reasonable: {avg_workflow_time:.3f}s"
        assert (
            max_workflow_time < 2.0
        ), f"Maximum workflow time should be reasonable: {max_workflow_time:.3f}s"
...
class TestMCPStressPerformance:
    ...
    async def test_maximum_concurrent_connections(self, mcp_server):
        ...
        try:
            ...
            for i in range(1000):  # Try up to 1000 clients
                ...
                success = await client.connect_server(
                    f"test_server_{i}", MCPConnectionType.HTTP, "http://localhost:8000"
                )
        ...
        assert (
            max_clients > 10
        ), f"Should support at least 10 concurrent connections: {max_clients}"
    ...
    async def test_maximum_throughput(self, mcp_server, mcp_client):
        ...
        for batch_size in batch_sizes:
            ...
            while time.time() - start_time < 10:
                ...
                for i in range(batch_size):
                    task = mcp_client.call_tool(
                        "get_inventory",
                        {"item_id": f"ITEM{completed_operations + i:06d}"},
                    )
                    tasks.append(task)
        ...
        print(
            f"Maximum throughput: {max_throughput:.2f} ops/sec (batch size: {optimal_batch_size})"
        )

        # Assertions
        assert (
            max_throughput > 50
        ), f"Should achieve reasonable maximum throughput: {max_throughput:.2f} ops/sec"
    ...
    async def test_memory_under_extreme_load(self, mcp_server, mcp_client):
        ...
        print(
            f"Extreme Load - Memory increase: {memory_increase:.1f}MB, Success rate: {success_rate:.2%}"
        )
        ...
        assert (
            success_rate > 0.5
        ), f"Should maintain reasonable success rate: {success_rate:.2%}"
    ...
    async def test_sustained_load_stability(self, mcp_server, mcp_client):
        ...
        while time.time() - start_time < test_duration:
            ...
            if int(time.time() - start_time) % 30 == 0:
                ...
                memory_samples.append(current_memory)
                print(
                    f"Time: {int(time.time() - start_time)}s, Memory: {current_memory:.1f}MB, Ops: {completed_operations}"
                )
        ...
        print(
            f"Sustained Load - Duration: {total_time:.1f}s, Throughput: {throughput:.2f} ops/sec, Success: {success_rate:.2%}, Memory stable: {memory_stable}"
        )

        # Assertions
        assert (
            throughput > 5
        ), f"Should maintain reasonable throughput: {throughput:.2f} ops/sec"
        assert (
            success_rate > 0.7
        ), f"Should maintain good success rate: {success_rate:.2%}"
