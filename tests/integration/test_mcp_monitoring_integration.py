...
class TestMCPMetricsCollection:
    ...
    async def test_metrics_recording(self, monitoring_service):
        ...
        await monitoring_service.metrics_collector.record_metric(
            "tool_executions", 1.0, MetricType.COUNTER, {"tool_name": "get_inventory"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "tool_execution_time",
            0.5,
        ...
        await monitoring_service.metrics_collector.record_metric(
            "active_connections", 1.0, MetricType.GAUGE, {"service": "mcp_server"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "memory_usage", 0.6, MetricType.GAUGE, {"component": "mcp_server"}
        )

        # Retrieve metrics
        tool_executions = await monitoring_service.metrics_collector.get_metric(
            "tool_executions", {"tool_name": "get_inventory"}
        )
        execution_times = await monitoring_service.metrics_collector.get_metric(
            "tool_execution_time", {"tool_name": "get_inventory"}
        )
        connections = await monitoring_service.metrics_collector.get_metric(
            "active_connections", {"service": "mcp_server"}
        )
        memory = await monitoring_service.metrics_collector.get_metric(
            "memory_usage", {"component": "mcp_server"}
        )
    ...
    async def test_metrics_aggregation(self, monitoring_service):
        ...
        response_times_summary = (
            await monitoring_service.metrics_collector.get_metric_summary(
                "response_time", {"endpoint": "api"}
            )
        )
        error_counts_summary = (
            await monitoring_service.metrics_collector.get_metric_summary(
                "error_count", {"error_type": "validation"}
            )
        )

        # Verify aggregation
        assert (
            response_times_summary.get("count", 0) > 0
        ), "Should aggregate response time metrics"
        assert (
            error_counts_summary.get("count", 0) > 0
        ), "Should aggregate error count metrics"

        # Check that we have multiple data points
        assert (
            response_times_summary.get("count", 0) >= 100
        ), "Should have all recorded response times"
        assert (
            error_counts_summary.get("count", 0) >= 5
        ), "Should have recorded error counts (5 errors in 100 iterations)"
    ...
    async def test_metrics_filtering(self, monitoring_service):
        ...
        await monitoring_service.metrics_collector.record_metric(
            ...
            1.0,
            MetricType.COUNTER,
            {"tool_name": "get_inventory", "agent": "equipment"},
        )
        await monitoring_service.metrics_collector.record_metric(
            "tool_executions",
            1.0,
            MetricType.COUNTER,
            {"tool_name": "get_orders", "agent": "operations"},
        )
        await monitoring_service.metrics_collector.record_metric(
            "tool_executions",
            1.0,
            MetricType.COUNTER,
            {"tool_name": "get_safety", "agent": "safety"},
        )

        # Filter by tool name
        inventory_metric = await monitoring_service.metrics_collector.get_metric(
            "tool_executions", {"tool_name": "get_inventory", "agent": "equipment"}
        )
        assert inventory_metric is not None, "Should filter by tool name"

        # Filter by agent
        equipment_metric = await monitoring_service.metrics_collector.get_metric(
            "tool_executions", {"tool_name": "get_inventory", "agent": "equipment"}
        )
    ...
    async def test_metrics_time_range(self, monitoring_service):
        ...
        with patch("src.api.services.mcp.monitoring.datetime") as mock_datetime:
            ...
            await monitoring_service.metrics_collector.record_metric(
                "old_metric", 1.0, MetricType.GAUGE, {}
            )

            mock_datetime.utcnow.return_value = one_hour_ago
            await monitoring_service.metrics_collector.record_metric(
                "recent_metric", 1.0, MetricType.GAUGE, {}
            )

            mock_datetime.utcnow.return_value = now
            await monitoring_service.metrics_collector.record_metric(
                "current_metric", 1.0, MetricType.GAUGE, {}
            )

        # Filter by time range
        recent_metrics = await monitoring_service.metrics_collector.get_metric_summary(
            "recent_metric"
        )
    ...
    async def test_metrics_retention(self, monitoring_service):
        ...
        for i in range(1000):
            await monitoring_service.metrics_collector.record_metric(
                "test_metric", float(i), MetricType.GAUGE, {"index": str(i)}
            )
    ...
    async def test_metrics_performance(self, monitoring_service):
        ...
        for i in range(10000):
            await monitoring_service.metrics_collector.record_metric(
                "performance_test", float(i), MetricType.GAUGE, {"iteration": str(i)}
            )
        ...
        print(
            f"Metrics Recording Performance - Time: {recording_time:.3f}s, Throughput: {throughput:.2f} metrics/sec"
        )

        # Assertions
        assert (
            throughput > 1000
        ), f"Should record metrics quickly: {throughput:.2f} metrics/sec"
...
class TestMCPHealthMonitoring:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="Requires MCPClient.connect() method and external services"
    )
    async def test_health_check_monitoring(
        self, monitoring_service, mcp_server, mcp_client
    ):
        ...
        await monitoring_service.metrics_collector.record_metric(
            ...
            1.0,
            MetricType.GAUGE,
            {"service": "mcp_server", "status": "healthy"},
        )
        await monitoring_service.metrics_collector.record_metric(
            "health_check",
            1.0,
            MetricType.GAUGE,
            {"service": "mcp_client", "status": "healthy"},
        )

        # Get health metrics
        health_metrics = await monitoring_service.metrics_collector.get_metric_summary(
            "health_check"
        )
    ...
    async def test_alert_threshold_monitoring(self, monitoring_service):
        ...
        await monitoring_service.metrics_collector.record_metric(
            "error_rate", 0.15, MetricType.GAUGE, {"service": "mcp_server"}
        )  # Exceeds 0.1 threshold
        await monitoring_service.metrics_collector.record_metric(
            "response_time", 6.0, MetricType.HISTOGRAM, {"endpoint": "api"}
        )  # Exceeds 5.0 threshold
        await monitoring_service.metrics_collector.record_metric(
            "memory_usage", 0.9, MetricType.GAUGE, {"component": "mcp_server"}
        )  # Exceeds 0.8 threshold
    ...
    async def test_service_health_monitoring(
        ...
        health_metrics = await monitoring_service.metrics_collector.get_metrics_by_name(
            "service_health"
        )
        ...
        assert (
            "services_healthy" in dashboard["health"]
        ), "Should track healthy services count"
    ...
    async def test_resource_monitoring(self, monitoring_service):
        ...
        await monitoring_service.metrics_collector.record_metric(
            "cpu_usage", 0.5, MetricType.GAUGE, {"component": "mcp_server"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "memory_usage", 0.6, MetricType.GAUGE, {"component": "mcp_server"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "disk_usage", 0.3, MetricType.GAUGE, {"component": "mcp_server"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "network_usage", 0.4, MetricType.GAUGE, {"component": "mcp_server"}
        )

        # Get resource metrics - use get_metrics_by_name to get all metrics regardless of labels
        cpu_metrics = await monitoring_service.metrics_collector.get_metrics_by_name(
            "cpu_usage"
        )
        memory_metrics = await monitoring_service.metrics_collector.get_metrics_by_name(
            "memory_usage"
        )
        disk_metrics = await monitoring_service.metrics_collector.get_metrics_by_name(
            "disk_usage"
        )
        network_metrics = (
    ...
    async def test_alert_escalation(self, monitoring_service):
        ...
        await monitoring_service.metrics_collector.record_metric(
            "error_rate", 0.05, MetricType.GAUGE, {"service": "mcp_server"}
        )  # Normal
        await monitoring_service.metrics_collector.record_metric(
            "error_rate", 0.12, MetricType.GAUGE, {"service": "mcp_server"}
        )  # Warning
        await monitoring_service.metrics_collector.record_metric(
            "error_rate", 0.25, MetricType.GAUGE, {"service": "mcp_server"}
        )  # Critical
    ...
    async def test_health_recovery_monitoring(self, monitoring_service):
        ...
        await monitoring_service.metrics_collector.record_metric(
            ...
            0.0,
            MetricType.GAUGE,
            {"service": "mcp_server", "status": "down"},
        )
        await monitoring_service.metrics_collector.record_metric(
            "service_health",
            0.0,
        ...
        assert "health" in dashboard, "Should track system health"
        assert (
            "overall_status" in dashboard["health"]
...
class TestMCPLoggingIntegration:
    ...
    async def test_audit_trail_logging(
        ...
        # Get audit trail
        audit_metrics = await monitoring_service.metrics_collector.get_metric_summary(
            "audit_event"
        )
    ...
    async def test_error_logging(self, monitoring_service):
        ...
        # Get error logs
        error_metrics = await monitoring_service.metrics_collector.get_metric_summary(
            "error_log"
        )
    ...
    async def test_structured_logging(self, monitoring_service):
        ...
        await monitoring_service.metrics_collector.record_metric(
            "structured_log", 1.0, MetricType.GAUGE, log_labels
        )

        # Get structured logs - use get_metrics_by_name to get list of Metric objects
        log_metrics = await monitoring_service.metrics_collector.get_metrics_by_name(
            "structured_log"
        )
    ...
    async def test_log_aggregation(self, monitoring_service):
        ...
        for i in range(100):
            ...
            await monitoring_service.metrics_collector.record_metric(
                "log_entry", 1.0, MetricType.GAUGE, log_labels
            )
...
class TestMCPPerformanceMonitoring:
    ...
    async def test_response_time_monitoring(
        ...
        for i in range(50):
            ...
            if result.success:
                ...
                await monitoring_service.metrics_collector.record_metric(
                    "response_time",
                    response_time,
                    {"endpoint": "tool_execution", "tool_name": "get_inventory"},
                )

        # Get response time metrics
        response_times = await monitoring_service.metrics_collector.get_metric_summary(
            "response_time"
        )
        ...
        assert (
            avg_time < 1.0
        ), f"Average response time should be reasonable: {avg_time:.3f}s"
        assert (
            max_time < 2.0
        ), f"Maximum response time should be reasonable: {max_time:.3f}s"
    ...
    async def test_throughput_monitoring(
        ...
        for batch in range(10):
            ...
            tasks = []
            for i in range(10):
                task = mcp_client.execute_tool(
                    "get_inventory", {"item_id": f"ITEM{batch * 10 + i:03d}"}
                )
                tasks.append(task)
            ...
            await monitoring_service.metrics_collector.record_metric(
                "throughput",
                batch_throughput,
                {"time_window": "batch", "batch_number": str(batch)},
            )
        ...
        await monitoring_service.metrics_collector.record_metric(
            "overall_throughput",
            overall_throughput,
            {"time_window": "total", "operations": str(operations_completed)},
        )

        # Get throughput metrics
        throughput_metrics = (
            await monitoring_service.metrics_collector.get_metric_summary("throughput")
        )
        overall_metrics = await monitoring_service.metrics_collector.get_metric_summary(
            "overall_throughput"
        )
    ...
    async def test_resource_utilization_monitoring(self, monitoring_service):
        ...
        for i in range(20):
            ...
            await monitoring_service.metrics_collector.record_metric(
                ...
                cpu_percent,
                MetricType.GAUGE,
                {"component": "mcp_server", "measurement": str(i)},
            )
        ...
        print(
            f"Resource Monitoring - Avg CPU: {avg_cpu:.1f}%, Avg Memory: {avg_memory:.1f}MB"
        )
    ...
    async def test_error_rate_monitoring(
        ...
        # Record error rate
        await monitoring_service.metrics_collector.record_metric(
            "error_rate", error_rate, {"service": "mcp_server", "time_window": "test"}
        )

        # Get error rate metrics
        error_rate_metrics = (
            await monitoring_service.metrics_collector.get_metric_summary("error_rate")
        )
    ...
    async def test_concurrent_operations_monitoring(
        ...
        assert (
            concurrency_metrics.get("count", 0) > 0
        ), "Should monitor concurrent operations"
...
class TestMCPSystemDiagnostics:
    ...
    async def test_system_health_dashboard(
        ...
        # Record various metrics
        await monitoring_service.metrics_collector.record_metric(
            "system_health", 1.0, MetricType.GAUGE, {"component": "mcp_server"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "system_health", 1.0, MetricType.GAUGE, {"component": "mcp_client"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "active_connections", 1.0, MetricType.GAUGE, {"service": "mcp_server"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "tool_executions", 10.0, MetricType.COUNTER, {"tool_name": "get_inventory"}
        )
    ...
    async def test_diagnostic_metrics(self, monitoring_service):
        ...
        await monitoring_service.metrics_collector.record_metric(
            ...
            1.0,
            MetricType.GAUGE,
            {"check_type": "memory", "status": "healthy", "component": "mcp_server"},
        )
        ...
        await monitoring_service.metrics_collector.record_metric(
            ...
            0.0,
            MetricType.GAUGE,
            {"check_type": "disk_space", "status": "warning", "component": "storage"},
        )

        # Get diagnostic metrics - use get_metrics_by_name to get list of Metric objects
        diagnostic_metrics = (
            await monitoring_service.metrics_collector.get_metrics_by_name("diagnostic")
        )
        ...
        healthy_checks = [
            m for m in diagnostic_metrics if m.labels.get("status") == "healthy"
        ]
        warning_checks = [
            m for m in diagnostic_metrics if m.labels.get("status") == "warning"
        ]
    ...
    async def test_troubleshooting_metrics(self, monitoring_service):
        ...
        assert (
            troubleshooting_metrics.get("count", 0) > 0
        ), "Should collect troubleshooting metrics"
    ...
    async def test_performance_bottleneck_detection(self, monitoring_service):
        ...
        assert (
            bottleneck_metrics.get("count", 0) > 0
    ...
    async def test_system_capacity_monitoring(self, monitoring_service):
        ...
        await monitoring_service.metrics_collector.record_metric(
            ...
            0.6,
            MetricType.GAUGE,
            {"resource_type": "cpu", "current_usage": "60.0", "max_capacity": "100.0"},
        )
