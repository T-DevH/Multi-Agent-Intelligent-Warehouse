...
class TestEquipmentAgentWorkflows:
    ...
    async def mock_equipment_tools(self, setup_mcp_services):
        ...
        await discovery.register_discovery_source(
            "equipment_adapter", mock_adapter, "mcp_adapter"
        )
        await asyncio.sleep(1)  # Allow discovery to run

        return mock_adapter

    async def test_equipment_status_check_workflow(
        self, equipment_agent, mock_equipment_tools
    ):
    ...
    async def test_equipment_maintenance_scheduling_workflow(
        self, equipment_agent, mock_equipment_tools
    ):
        ...
        assert result["equipment_id"] == "EQ001", "Should return correct equipment ID"
        assert (
            result["scheduled_date"] == "2024-02-15"
        ), "Should return correct scheduled date"

    async def test_equipment_history_workflow(
        self, equipment_agent, mock_equipment_tools
    ):
    ...
    async def test_equipment_location_update_workflow(
        self, equipment_agent, mock_equipment_tools
    ):
    ...
    async def test_equipment_workflow_with_validation(
        self, equipment_agent, mock_equipment_tools, setup_mcp_services
    ):
    ...
    async def test_equipment_workflow_error_handling(
        self, equipment_agent, mock_equipment_tools
    ):
    ...
    async def test_equipment_workflow_performance(
        self, equipment_agent, mock_equipment_tools
    ):
        ...
        assert (
            execution_time < 5.0
        ), f"Should complete within reasonable time: {execution_time:.2f}s"
...
class TestOperationsAgentWorkflows:
    ...
    async def mock_operations_tools(self, setup_mcp_services):
        ...
        # Register adapter
        await discovery.register_discovery_source(
            "operations_adapter", mock_adapter, "mcp_adapter"
        )
        await asyncio.sleep(1)

        return mock_adapter

    async def test_work_order_creation_workflow(
        self, operations_agent, mock_operations_tools
    ):
    ...
    async def test_operation_scheduling_workflow(
        self, operations_agent, mock_operations_tools
    ):
        ...
        assert (
            result["operation_type"] == "picking"
        ), "Should return correct operation type"
        assert (
            result["scheduled_time"] == "2024-01-21T14:00:00Z"
        ), "Should return correct scheduled time"

    async def test_workflow_optimization_workflow(
        self, operations_agent, mock_operations_tools
    ):
    ...
    async def test_workload_status_workflow(
        self, operations_agent, mock_operations_tools
    ):
...
class TestSafetyAgentWorkflows:
    ...
    async def mock_safety_tools(self, setup_mcp_services):
        ...
        # Register adapter
        await discovery.register_discovery_source(
            "safety_adapter", mock_adapter, "mcp_adapter"
        )
        await asyncio.sleep(1)

        return mock_adapter

    async def test_safety_compliance_check_workflow(
        self, safety_agent, mock_safety_tools
    ):
    ...
    async def test_safety_incident_reporting_workflow(
        self, safety_agent, mock_safety_tools
    ):
        ...
        assert (
            result["incident_type"] == "near_miss"
        ), "Should return correct incident type"
        assert result["severity"] == "low", "Should return correct severity"

    async def test_safety_training_scheduling_workflow(
        self, safety_agent, mock_safety_tools
    ):
        ...
        assert (
            result["training_type"] == "forklift_safety"
        ), "Should return correct training type"
        assert (
            len(result["participants"]) == 2
        ), "Should include correct number of participants"
...
class TestCrossAgentCollaboration:
    ...
    async def test_incident_response_workflow(self, all_agents):
        ...
        ), "Operations agent should create work order"
