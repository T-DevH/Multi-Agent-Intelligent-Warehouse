...
class TestMCPDockerDeployment:
    ...
    async def test_docker_container_health_monitoring(
        ...
        await monitoring_service.metrics_collector.record_metric(
            "container_health", 1.0, MetricType.GAUGE, {"container": "mcp_server"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "active_connections", 1.0, MetricType.GAUGE, {"container": "mcp_server"}
        )
...
class TestMCPKubernetesDeployment:
    ...
    async def test_kubernetes_service_discovery(
        self, mcp_server, mcp_client, service_registry
    ):
    ...
    async def test_kubernetes_configuration_management(
        ...
        new_config = ToolDiscoveryConfig(
            discovery_interval=10, max_tools_per_source=200
        )

        # Update configuration
        discovery_service.config = new_config
        assert (
            discovery_service.config.discovery_interval == 10
        ), "Should update configuration"

        # Test configuration persistence
        assert (
            discovery_service.config.max_tools_per_source == 200
    ...
    async def test_kubernetes_monitoring_integration(
        self, mcp_server, mcp_client, monitoring_service
    ):
        ...
        await monitoring_service.metrics_collector.record_metric(
            "pod_health", 1.0, MetricType.GAUGE, {"pod": "mcp-server-001"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "request_count", 1.0, MetricType.COUNTER, {"pod": "mcp-server-001"}
        )
...
class TestMCPProductionDeployment:
    ...
    async def test_production_monitoring(
        ...
        await monitoring_service.metrics_collector.record_metric(
            "error_rate", 0.01, MetricType.GAUGE, {"environment": "production"}
        )
        await monitoring_service.metrics_collector.record_metric(
            "response_time", 0.5, MetricType.GAUGE, {"environment": "production"}
        )
    ...
    async def test_production_performance(self, mcp_server, mcp_client):
        ...
        assert (
            throughput > 10
        ), f"Should maintain good throughput: {throughput:.2f} ops/sec"
        assert (
            execution_time < 60
    ...
    async def test_production_backup_and_recovery(
        self, mcp_server, mcp_client, discovery_service
    ):
        ...
        await discovery_service.register_discovery_source(
            "test_adapter", mock_adapter, "mcp_adapter"
        )
    ...
    async def test_production_logging_and_auditing(
        self, mcp_server, mcp_client, monitoring_service
    ):
